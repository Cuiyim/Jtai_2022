function player(config) {
    if (config.url.indexOf(".m3u8") > 0 || config.url.indexOf(".mp4") > 0 || config.url.indexOf(".mkv") > 0 || config.url.indexOf(".flv") > 0) {
        MPlayer(config.url, config.title, config.vkey, config.next);
    } else {
        $.ajaxSettings.timeout = '30000';
        $.post("https://tenapi.cn/video/?url=", {
                "url": config.url
            },
            function (data) {
                if (data.code == "200") {
...
console.log("\n %c z8  %c io","color:#fff;background:linear-gradient(90deg,#448bff,#44e9ff);padding:5px 0;","color:#000;background:linear-gradient(90deg,#44e9ff,#ffffff);padding:5px 10px 5px 0px;");
